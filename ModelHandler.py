...
class ModelHandler(nn.Module):
    ...
    def forword(self, features, lengths):
        return self.dnn(features, lengths)
    ...
    def fit(self, X_train, y_train, loss_fn, optimizer, modelPath=None, eval_set=None, early_stopping_rounds=None,
    ...
    def predict_proba(self, testDF, inputType='tensor', temperature=1):
        ...
        if inputType == 'tensor':
            ...
            return torch.sigmoid(
                self.forword(torch.tensor(np.array(testDF), dtype=torch.float32, device=self.device)) / temperature)
        torch.set_grad_enabled(True)
...
#                 # l2Loss = l2Loss + regularization_loss(self.model)
#
#                 self.train()
#                 torch.set_grad_enabled(True)
#                 loss.backward()
...
#                 print('train loss:', trainLoss / float(batchNumInEveryEpoch))
#                 # print('train l2_loss:', l2Loss / float(batchNumInEveryEpoch))
#
...
#         accracy = np.mean((torch.argmax(y_score, 1) == y_true).numpy())
#         return accracy
